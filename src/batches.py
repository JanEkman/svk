...
def add_sample_normality2(df, cols, ws, delay =1):

    dfc   = df[cols]
    # dfc   = dfc.dropna()
    rollc = dfc.rolling(window=ws, min_periods=ws).cov()
    rollm = dfc.rolling(window=ws, min_periods=ws).mean()
    norm  = list(np.ones(ws))

    for i in range(ws, len(dfc)):
        cov  = rollc.loc[i - delay]
        xm   = rollm.loc[i - delay].values
        xz   = dfc.loc[i].values
        norm += [normality(xz, xm, cov, ws)]

    norm = pd.DataFrame(norm, columns = ['norm'])
...
def alarm2(df, p, cols, ws, delay =1):
    print(cols)
    df = add_sample_normality2(df, cols, ws, delay)
    df.is_copy = False
    df['alarm'] = df['norm'] < p
    df['alarm'] = df['alarm'].apply(int)
